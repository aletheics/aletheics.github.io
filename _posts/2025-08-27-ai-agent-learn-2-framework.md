...
上述提到两种方式定义工具，示例如下（示例中都提供给代码智能体`CodeAgent`使用）：

* 方式1：`@tool`装饰器方式，用于定义简单工具。需要定义包含以下要素的函数：
...
* 方式2：通过Python类定义工具（创建`Tool`的子类）。用于定义复杂工具，可以通过类封装函数及其元数据来帮助LLM理解使用方式，类中需要定义：
    * name: 工具名称
...
### 3.3. 实验：工具调用型智能体（ToolCallingAgent）
