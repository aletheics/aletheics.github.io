...
>
> zStorage 是一个存储平台，可作为数据库产品和存储产品的底座。

1、[丝析发解丨zStorage 是如何保持性能稳步上升的?](https://www.modb.pro/db/1762660180899205120)

* 作者介绍：张洋，云和恩墨 zStorage 性能架构师，负责 zStorage 产品的性能优化和例行看护工作。
...
* 自动化措施
    ...
    * 3、时延分析和对比：zStorage 内置了点位时延分析工具(zTrace)，能够详细分析出一个IO请求在某个模块中的耗时情况。

2、[分布式存储系统性能调优 - zStorage性能进化历程概述](https://zhuanlan.zhihu.com/p/692175522) (**很精彩**)

* 作者介绍：小川（知乎名），好像也是上面张洋大佬的知乎号
* 先提下基于的硬件配置简介：
    * 1）`3个存储节点`，每节点两张`100Gbits IB卡`，8块高速NVME SSD，两个至强 4314CPU，12根16GB的内存条
    * 2）总共32个CPU物理核心，zStorage进程实际占用14个物理核心，空余18个物理核心将来备用
    * 3）`2个计算节点`用于下发IO负载，每节点同样搭载两张100Gbits IB卡。
* zStorage的性能进化历程的`四个阶段`，可从中参考学习：
    * **1）子系统单独调优**：在这个阶段各个子系统，包括`FrontEnd`、`Raft`、`LocalStore`对各自的代码进行单独调优，达到各自既定的目标
        * IO路径上三个主要的子系统就是上面的`FrontEnd`、`Raft`、`LocalStore`，根据总目标90万IOPS各自定目标，比如：FrontEnd达到200万，Raft达到100万，LocalStore达到50万
            * FrontEnd的主要作用是提供`nvmf（NVMe over Fabric）`、`iscsi`这样的标准存储协议，将接收到的请求路由到后端的Raft
            * Raft的主要作用是实现数据三副本，保证数据副本的一致性
            * LocalStore作为IO路径的最后一环，组织数据在硬盘上的结构形式，完成数据下盘持久化
                * 简要优化记录，可见博客记录：[Linux C 性能优化实战（基于SPDK框架）](https://weibo.com/1202332555/KyDuKB3hY)
                * 单线程切换为多线程、缓存底层存储对象、笔记本涉及降频问题，
                * perf看热点在CRC和内存拷贝较耗时，尝试优化（Intel SSE 无果）、
                * strace查看是否有系统调用（无）、继续perf检查热点函数，优化函数（无明显提升）、去掉CRC（提升）、`-O2`编译优化（提升）、
                * review代码线程模型改造得更紧凑（无明显提升）；
                * 继续perf查热点，很多openssl的函数调用，热点在MD5函数，分析流程发现一个4K的IO，会导致一个16K的额外IO，并做了Md5校验。**缓存这些16KB数据**，减少了内存拷贝，同时减少了Md5校验，**提升突飞猛进**
                * 优化热点函数，强制内联一些perf跟踪到的热点函数（有提升）
                * 一些实验，如哈希表大小调整，优化哈希表以提高CPU缓存命中率，性能反而降低（不符合预期，重新分析预期）
        * 工具：改造`fio`，让其支持压测某一个代码模块
    * **2）全流程集成调优**：将各个子系统调优的成果真正**体现到产品**上，达到zStorage第一个版本的性能目标“4k 7:3混合随机读写90万IOPS”
        * 实现了两个非常重要的性能分析工具，不仅在这个阶段起到了重要作用，而且一直伴随着后续的全部调优阶段
            * `zTrace`：主要作用是统计代码流程或者模块的时延、IOPS、以及ifio（正在处理的IO数量）等信息。
                * 通过这个工具可以看见各个流程的耗时情况，用于诊断性能问题出现在哪个环节，找到瓶颈所在。
            * `打环工具`：主要目的是在IO路径上排除某一个子系统，以此确认问题出现在哪个环节
        * 这个阶段主要是对上一个阶段“子系统单独调优”的成果做验证，通过以上两个主要工具和其他一些工具找到瓶颈。**反复在“子系统单独调优”和“全流程集成调优”中迭代进行，互相促进**。这个阶段历史3个月，达到了既定的90万IOPS目标。
    * **3）持续性能调优，强化性能优势**：该阶段由专人分析性能问题，再由各个开发组配合完成优化
        * 2022年初性能目标达到了既定目标90万IOPS之后，zStorage原型机的最后一环完成
        * 但是随着功能特性的完善和BUG修复，代码越来越多，势必对性能造成影响，2022年底IOPS已经守不住90万这个水平线了
        * 在这个背景下，**新一轮性能调优展开**。
            * 不像之前倾力投入，而是确立性能调优负责人，由负责人牵头分析性能问题，然后协调各个开发组投入部分人力，配合解决，优化性能。
            * 这个阶段是一个持续提升，缓慢爬坡的过程，持续了2023一整年
        * 这阶段性能优化内容分享，具体见下面的 《分布式存储系统性能调优-zStorage是如何将IOPS从100万提升到200万的？》
    * **4）针对应用场景的性能优化**：该阶段更多的是结合应用场景，做针对性的优化，而不再局限于简单的性能跑分数据
        * 分布式存储系统的性能最终是要服务于应用需求的
        * 几个例子
            * 数据库场景对8K顺序/随机读的带宽需求
                * 常理：8K大小的IO属于小IO，应该着重考虑时延和IOPS数据
                * 但是一个需求场景：需要8K读IO能够将两个计算节点的**带宽打满**
            * 数据库场景对IO优先级的区分需求
                * 数据库的写日志性能和其他IO区分，实现高低优先级队列，将写日志的IO放到高优先级队列，其他IO放到低优先级队列
            * 故障场景下，存储系统的可用性需求
    * **未来第五阶段**的性能优化：有一部分是第四阶段的延续，另外一部分是由于新特性的引入：例如：压缩、EC、写放大优化等，需要针对性的做性能调优
        * 分布式存储系统性能调优的本质是解决一个又一个的瓶颈
        * 硬盘、网络、CPU、内存、插槽等部件是否达到了瓶颈
            * 如果硬盘达到了瓶颈，应该寻求加盘，或者减少写放大等方式解决；
            * 如果内存达到了瓶颈，应该增加内存条，充分利用内存通道带宽，或者优化程序的内存访问模式，减少不必要的内存访问等
        * 总结来讲就是**充分利用硬件能力，另外是高效利用硬件能力，即对硬件资源的利用能有效的转化为用户的价值。**
...
[常用性能优化手段及在风控系统中的应用](https://mp.weixin.qq.com/s/AVJENJFA6_vRopU4FLBpuw)
