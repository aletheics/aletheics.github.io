...
[TCP半连接全连接（三） -- eBPF跟踪全连接队列溢出（上）](https://aletheics.github.io/2024/06/23/bcctools-trace-tcp_connect/) 这篇中，自己要写eBPF程序了，带着实践的目的去参考已有工具代码，发现视角完全变了。
...
（关于`TCP_NEW_SYN_RECV`状态，我们在[TCP半连接全连接（二） -- 半连接队列代码逻辑](https://aletheics.github.io/2024/05/30/tcp_syn_queue/)中还分析过netstat里是追踪不到的）
