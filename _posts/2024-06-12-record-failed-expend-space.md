...
在“[eBPF学习实践系列（二） -- bcc tools网络工具集](https://aletheics.github.io/2024/06/10/bcc-tools-network/)”里，实验`tcpretrans`时想用`tc`构造重传的场景。需要安装`kernel-modules-extra`，提示`/boot`空间不够了，之前装系统只分配了220M左右（Windows+CentOS8.5的双系统）。
